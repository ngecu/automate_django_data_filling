    ...
    def dump_inline_table(self, section):
        ...
        if isinstance(section, dict):
            ...
            for k, v in section.items():
                val = dump_inline_table(v)
                val_list.append(k + " = " + val)
            retval += "{ " + ", ".join(val_list) + " }\n"
            return retval
        else:
            return unicode(dump_value(section))
                ...
                if arrayoftables:
                    for a in o[section]:
                        ...
                        while d:
                            newd = _dict()
                            for dsec in d:
                                s1, d1 = dump_sections(d[dsec], sup +
                                                            qsection + "." +
                                                            dsec)
                                if s1:
                    ...
                    if o[section] is not None:
                        retstr += (qsection + " = " +
                                   unicode(dump_value(o[section])) + '\n')
            elif preserve and isinstance(o[section], InlineTableDict):
                retstr += (qsection + " = " +
                           dump_inline_table(o[section]))
            else:
...
class TomlNumpyEncoder(TomlEncoder):
    ...
    def __init__(self, _dict=dict, preserve=False):
        ...
        dump_funcs[np.float16] = _dump_float
        dump_funcs[np.float32] = _dump_float
        dump_funcs[np.float64] = _dump_float
        dump_funcs[np.int16] = _dump_int
        dump_funcs[np.int32] = _dump_int
        dump_funcs[np.int64] = _dump_int
...
class TomlPreserveCommentEncoder(TomlEncoder):
    ...
    def __init__(self, _dict=dict, preserve=False):
        ...
        dump_funcs[CommentValue] = lambda v: v.dump(dump_value)
