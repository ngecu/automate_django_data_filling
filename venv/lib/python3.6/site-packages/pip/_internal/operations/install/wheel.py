...
class ZipBackedFile(object):
    def __init__(self, src_record_path, dest_path, zip_file):
        ...
        src_record_path = src_record_path
        dest_path = dest_path
        _zip_file = zip_file
        changed = False
    ...
    def _getinfo(self):
        # type: () -> ZipInfo
        if not PY2:
            return _zip_file.getinfo(src_record_path)
        ...
        return _zip_file.getinfo(src_record_path.encode("utf-8"))
    ...
    def save(self):
        ...
        with _zip_file.open(zipinfo) as f:
            with open(dest_path, "wb") as dest:
                shutil.copyfileobj(f, dest)

        if zip_item_is_executable(zipinfo):
            set_extracted_file_to_default_mode_plus_executable(dest_path)
...
class ScriptFile(object):
    def __init__(self, file):
        # type: (File) -> None
        _file = file
        src_record_path = _file.src_record_path
        dest_path = _file.dest_path
        changed = False

    def save(self):
        # type: () -> None
        _file.save()
        changed = fix_script(dest_path)
