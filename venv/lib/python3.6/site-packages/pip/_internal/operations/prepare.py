...
class RequirementPreparer(object):
    ...
    def __init__(
        ...
        _session = session
        _download = Downloader(session, progress_bar)
        _batch_download = BatchDownloader(session, progress_bar)
        finder = finder
        ...
        use_lazy_wheel = lazy_wheel

        # Memoized downloaded files, as mapping of url: (path, mime type)
        _downloaded = {}  # type: Dict[str, Tuple[str, str]]
    ...
    def _log_preparing_link(self, req):
        ...
        if (message, information) != _previous_requirement_header:
            _previous_requirement_header = (message, information)
    ...
    def _fetch_metadata_using_lazy_wheel(self, link):
        ...
        try:
            return dist_from_wheel_url(name, url, _session)
    ...
    def prepare_linked_requirement(self, req, parallel_builds=False):
        ...
        with indent_log():
            ...
            if download_dir is not None and link.is_wheel:
                hashes = _get_linked_req_hashes(req)
                file_path = _check_download_dir(req.link, download_dir, hashes)

            if file_path is not None:
                # The file is already available, so mark it as downloaded
                _downloaded[req.link.url] = file_path, None
            else:
                # The file is not available, attempt to fetch only metadata
                wheel_dist = _fetch_metadata_using_lazy_wheel(link)
                if wheel_dist is not None:
                    req.needs_more_preparation = True
                    return wheel_dist

            # None of the optimizations worked, fully prepare the requirement
            return _prepare_linked_requirement(req, parallel_builds)
    ...
    def prepare_linked_requirements_more(self, reqs, parallel_builds=False):
        ...
        _downloaded.update(_batch_download(links, tmpdir))
        for req in reqs:
            _prepare_linked_requirement(req, parallel_builds)
    ...
    def _prepare_linked_requirement(self, req, parallel_builds):
        ...
        hashes = _get_linked_req_hashes(req)
        if link.url not in _downloaded:
            try:
                local_file = unpack_url(
                    link, req.source_dir, _download,
                    download_dir, hashes,
                )
            ...
            file_path, content_type = _downloaded[link.url]
            if hashes:
        ...
        dist = _get_prepared_distribution(
            req, req_tracker, finder, build_isolation,
        )
        return dist
    ...
    def prepare_editable_requirement(
        ...
        with indent_log():
            ...
            req.ensure_has_source_dir(src_dir)
            req.update_editable(download_dir is None)
