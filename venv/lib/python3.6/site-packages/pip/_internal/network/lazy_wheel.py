...
class LazyZipOverHTTP(object):
    ...
    def __init__(self, url, session, chunk_size=CONTENT_CHUNK_SIZE):
        ...
        _session, _url, _chunk_size = session, url, chunk_size
        _length = int(head.headers['Content-Length'])
        _file = NamedTemporaryFile()
        truncate(_length)
        _left = []  # type: List[int]
        _right = []  # type: List[int]
    ...
    def read(self, size=-1):
        ...
        """
        download_size = max(size, _chunk_size)
        start, length = tell(), _length
        stop = length if size < 0 else min(start+download_size, length)
        start = max(0, stop-download_size)
        _download(start, stop-1)
    ...
    def _check_zip(self):
        ...
        for start in reversed(range(0, end, _chunk_size)):
            _download(start, end)
            with _stay():
                try:
    ...
    def _stream_response(self, start, end, base_headers=HEADERS):
        ...
        return _session.get(_url, headers=headers, stream=True)
    ...
    def _merge(self, start, end, left, right):
        ...
        """
        lslice, rslice = _left[left:right], _right[left:right]
        ...
        if i <= end:
            yield i, end
        _left[left:right], _right[left:right] = [start], [end]
