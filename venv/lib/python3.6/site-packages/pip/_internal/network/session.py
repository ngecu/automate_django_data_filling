...
class PipSession(requests.Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pip_trusted_origins = []  # type: List[Tuple[str, Optional[int]]]

        # Attach our User Agent to the request
        headers["User-Agent"] = user_agent()

        # Attach our Authentication handler to the session
        auth = MultiDomainBasicAuth(index_urls=index_urls)
        ...
        if cache:
            ...
            _trusted_host_adapter = InsecureCacheControlAdapter(
                cache=SafeFileCache(cache),
                max_retries=retries,
            )
        else:
            secure_adapter = HTTPAdapter(max_retries=retries)
            _trusted_host_adapter = insecure_adapter
    ...
    def update_index_urls(self, new_index_urls):
        ...
        :param new_index_urls: New index urls to update the authentication
            handler with.
        """
        auth.index_urls = new_index_urls
    ...
    def add_trusted_host(self, host, source=None, suppress_logging=False):
        ...
        if host_port not in pip_trusted_origins:
            pip_trusted_origins.append(host_port)
    ...
    def iter_secure_origins(self):
        ...
        for secure_origin in SECURE_ORIGINS:
            yield secure_origin
        for host, port in pip_trusted_origins:
    ...
    def is_secure_origin(self, location):
        ...
        for secure_origin in iter_secure_origins():
