...
class Git(VersionControl):
    ...
    def is_immutable_rev_checkout(self, url, dest):
        # type: (str, str) -> bool
        _, rev_options = get_url_rev_options(hide_url(url))
        ...
        is_tag_or_branch = bool(
            get_revision_sha(dest, rev_options.rev)[0]
        )
    ...
    def fetch_new(self, dest, url, rev_options):
        ...
        run_command(make_command('clone', '-q', url, dest))

        if rev_options.rev:
            # Then a specific revision was requested.
            rev_options = resolve_revision(dest, url, rev_options)
            ...
            elif get_current_branch(dest) != branch_name:
    ...
    def update(self, dest, url, rev_options):
        ...
        if get_git_version() >= parse_version('1.9.0'):
            # fetch tags in addition to everything else
            run_command(['fetch', '-q', '--tags'], cwd=dest)
        else:
            run_command(['fetch', '-q'], cwd=dest)
