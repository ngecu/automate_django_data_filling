...
class RevOptions(object):
    ...
    def __repr__(self):
        # type: () -> str
        return '<RevOptions {}: rev={!r}>'.format(vc_class.name, rev)

    @property
    def arg_rev(self):
        # type: () -> Optional[str]
        if rev is None:
            return vc_class.default_arg_rev
    ...
    def to_args(self):
        ...
        rev = arg_rev
        if rev is not None:
            args += vc_class.get_base_rev_args(rev)
        args += extra_args
    ...
    def make_new(self, rev):
        ...
        """
        return vc_class.make_rev_options(rev, extra_args=extra_args)
...
class VcsSupport(object):
    ...
    def __init__(self):
        ...
        # systems
        urllib_parse.uses_netloc.extend(schemes)
        # Python >= 2.7.4, 3.3 doesn't have uses_fragment
        if getattr(urllib_parse, 'uses_fragment', None):
            urllib_parse.uses_fragment.extend(schemes)
        super(VcsSupport, self).__init__()

    def __iter__(self):
        # type: () -> Iterator[str]
        return _registry.__iter__()
    ...
    def dirnames(self):
        # type: () -> List[str]
        return [backend.dirname for backend in backends]
    ...
    def get_backend_for_dir(self, location):
        ...
        """
        vcs_backends = {}
        for vcs_backend in _registry.values():
...
class VersionControl(object):
    ...
    def get_url_rev_options(self, url):
        ...
        """
        secret_url, rev, user_pass = get_url_rev_and_auth(url.secret)
        username, secret_password = user_pass
        password = None  # type: Optional[HiddenText]
        if secret_password is not None:
            password = hide_value(secret_password)
        extra_args = make_rev_args(username, password)
        rev_options = make_rev_options(rev, extra_args=extra_args)
    ...
    def obtain(self, dest, url):
        ...
        :param url: the repository URL starting with a vcs prefix.
        """
        url, rev_options = get_url_rev_options(url)

        if not os.path.exists(dest):
            fetch_new(dest, url, rev_options)
            return

        rev_display = rev_options.to_display()
        if is_repository_directory(dest):
            existing_url = get_remote_url(dest)
            if compare_urls(existing_url, url.secret):
                logger.debug(
                ...
                if not is_commit_id_equal(dest, rev_options.rev):
                    logger.info(
                        'Updating %s %s%s',
                        display_path(dest),
                        repo_name,
