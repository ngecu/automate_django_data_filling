...
class Factory(object):
    def __init__(
        ...
        _finder = finder
        preparer = preparer
        _wheel_cache = wheel_cache
        _python_candidate = RequiresPythonCandidate(py_version_info)
        _make_install_req_from_spec = make_install_req
        _use_user_site = use_user_site
        _force_reinstall = force_reinstall
        _ignore_requires_python = ignore_requires_python

        _link_candidate_cache = {}  # type: Cache[LinkCandidate]
        _editable_candidate_cache = {}  # type: Cache[EditableCandidate]
    ...
    def _make_candidate_from_link(
        ...
        if template.editable:
            if link not in _editable_candidate_cache:
                _editable_candidate_cache[link] = EditableCandidate(
                    link, template, factory=self, name=name, version=version,
                )
            base = _editable_candidate_cache[link]  # type: BaseCandidate
        else:
            if link not in _link_candidate_cache:
                _link_candidate_cache[link] = LinkCandidate(
                    link, template, factory=self, name=name, version=version,
                )
            base = _link_candidate_cache[link]
        if extras:
            return ExtrasCandidate(base, extras)
        return base
    ...
    def _iter_found_candidates(
        ...
        if not _force_reinstall and name in _installed_dists:
            installed_dist = _installed_dists[name]
            if specifier.contains(installed_dist.version, prereleases=True):
                installed_candidate = _make_candidate_from_dist(
                    dist=installed_dist,
                    extras=extras,
                    template=template,
                )
        ...
        def iter_index_candidates():
            ...
            result = _finder.find_best_candidate(
                project_name=name,
                specifier=specifier,
                hashes=hashes,
            )
            ...
            for ican in reversed(list(result.iter_applicable())):
                yield _make_candidate_from_link(
                    link=ican.link,
                    extras=extras,
                    template=template,
                    name=name,
    ...
    def make_requirement_from_install_req(self, ireq, requested_extras):
        ...
        if ireq.link.is_wheel:
            ...
            if not wheel.supported(_finder.target_python.get_tags()):
                msg = "{} is not a supported wheel on this platform.".format(
                    wheel.filename,
                )
                raise UnsupportedWheel(msg)
        cand = _make_candidate_from_link(
            ireq.link,
            extras=frozenset(ireq.extras),
            template=ireq,
        ...
        return make_requirement_from_candidate(cand)
    ...
    def make_requirement_from_spec(
        ...
        ireq = _make_install_req_from_spec(specifier, comes_from)
        return make_requirement_from_install_req(ireq, requested_extras)

    def make_requires_python_requirement(self, specifier):
        # type: (Optional[SpecifierSet]) -> Optional[Requirement]
        if _ignore_requires_python or specifier is None:
            return None
        return RequiresPythonRequirement(specifier, _python_candidate)
    ...
    def get_wheel_cache_entry(self, link, name):
        ...
        """
        if _wheel_cache is None or preparer.require_hashes:
            return None
        return _wheel_cache.get_cache_entry(
            link=link,
            package_name=name,
    ...
    def get_dist_to_uninstall(self, candidate):
        ...
        dist = _installed_dists.get(candidate.name)
    ...
    def _report_requires_python_error(
        ...
        message = message_format.format(
            ...
            version=_python_candidate.version,
    ...
    def get_installation_error(self, e):
        ...
        for cause in e.causes:
            if isinstance(cause.requirement, RequiresPythonRequirement):
                return _report_requires_python_error(
                    cause.requirement,
                    cause.parent,
                )
