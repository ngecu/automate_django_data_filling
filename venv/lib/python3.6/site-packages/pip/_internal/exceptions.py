...
class NetworkConnectionError(PipError):
    ...
    def __init__(self, error_msg, response=None, request=None):
        ...
        request = request
        error_msg = error_msg
        if (response is not None and not request and
...
class HashErrors(InstallationError):
    ...
    def __str__(self):
        # type: () -> str
        lines = []
        errors.sort(key=lambda e: e.order)
        for cls, errors_of_cls in groupby(errors, lambda e: e.__class__):
...
class HashError(InstallationError):
    ...
    def body(self):
        ...
        """
        return '    {}'.format(_requirement_name())
    ...
    def _requirement_name(self):
        ...
        """
        return str(req) if req else 'unknown package'
...
class HashMissing(HashError):
    ...
    def body(self):
        ...
        if req:
            ...
            package = (req.original_link if req.original_link
                       # In case someone feeds something downright stupid
                       # to InstallRequirement's constructor.
                       else getattr(req, 'req', None))
...
class HashMismatch(HashError):
    ...
    def _hash_comparison(self):
        ...
        lines = []  # type: List[str]
        for hash_name, expecteds in iteritems(allowed):
            prefix = hash_then_or(hash_name)
            lines.extend(('        Expected {} {}'.format(next(prefix), e))
                         for e in expecteds)
            lines.append('             Got        {}\n'.format(
                         gots[hash_name].hexdigest()))
        return '\n'.join(lines)
...
class ConfigurationFileCouldNotBeLoaded(ConfigurationError):
    ...
    def __str__(self):
        ...
        if fname is not None:
            message_part = " in {}.".format(fname)
        else:
            assert error is not None
            message_part = ".\n{}\n".format(error)
        return "Configuration file {}{}".format(reason, message_part)
