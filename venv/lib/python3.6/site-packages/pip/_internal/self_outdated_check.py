...
class SelfCheckState(object):
    def __init__(self, cache_dir):
        ...
        if cache_dir:
            statefile_path = os.path.join(
                cache_dir, "selfcheck", _get_statefile_name(key)
            )
            try:
    ...
    def save(self, pypi_version, current_time):
        ...
        if not check_path_owner(os.path.dirname(statefile_path)):
            return

        # Now that we've ensured the directory is owned by this user, we'll go
        # ahead and make sure that all our directories are created.
        ensure_dir(os.path.dirname(statefile_path))
        ...
        with adjacent_tmp_file(statefile_path) as f:
            f.write(ensure_binary(text))

        try:
            # Since we have a prefix-specific state file, we can just
            # overwrite whatever is there, no need to check.
            replace(f.name, statefile_path)
        except OSError:
            # Best effort.
            pass
