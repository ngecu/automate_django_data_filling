...
class SourceDistribution(AbstractDistribution):
    ...
    def prepare_distribution_metadata(self, finder, build_isolation):
        ...
        should_isolate = req.use_pep517 and build_isolation
        if should_isolate:
    ...
    def _setup_isolation(self, finder):
        ...
        # requirements.
        pyproject_requires = req.pyproject_requires
        assert pyproject_requires is not None

        req.build_env = BuildEnvironment()
        req.build_env.install_requirements(
            finder, pyproject_requires, 'overlay',
            "Installing build dependencies"
        )
        conflicting, missing = req.build_env.check_requirements(
            req.requirements_to_check
        )
        if conflicting:
