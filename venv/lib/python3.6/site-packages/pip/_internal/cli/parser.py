...
class PrettyHelpFormatter(optparse.IndentedHelpFormatter):
    ...
    def format_option_strings(self, option):
        return _format_option_strings(option)
...
class UpdatingDefaultsHelpFormatter(PrettyHelpFormatter):
    ...
    def expand_default(self, option):
        ...
        if parser is not None:
            parser._update_defaults(parser.defaults)
...
class CustomOptionParser(optparse.OptionParser):
    ...
    def insert_option_group(self, idx, *args, **kwargs):
        ...
        group = add_option_group(*args, **kwargs)
...
class ConfigOptionParser(CustomOptionParser):
    ...
    def _get_ordered_configuration_items(self):
        ...
        override_order = ["global", name, ":env:"]

        # Pool the options into different groups
        section_items = {name: [] for name in override_order}
        for section_key, val in config.items():
            # ignore empty values
            if not val:
                logger.debug(
    ...
    def _update_defaults(self, defaults):
        ...
        for key, val in _get_ordered_configuration_items():
        ...
        for key in late_eval:
            defaults[key] = getattr(values, key)
        values = None
        return defaults
    ...
    def get_default_values(self):
        ...
        try:
            ...
            exit(UNKNOWN_ERROR, str(err))

        defaults = _update_defaults(defaults.copy())  # ours
        for option in _get_all_options():
